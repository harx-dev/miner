...
const SignUpForm = () => {
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    setErrorMessage(null); // Clear error before submission
    ...
    if (response.ok) {
      ...
      setErrorMessage(errorData.message); // Set the error message from Server
    }
  };
    ...
    <div className="w-full md:max-w-screen-sm">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
          ...
          {errorMessage && (
            <p className="text-red-500 text-sm mt-2 flex justify-center font-medium">
              {errorMessage}
            </p>
          )}
        </form>
